...
if [ "$color_prompt" = yes ]; then
	...
	twoline)
		# two line kali-style prompt with red cursor
		PS1=$prompt_color'┌──${debian_chroot:+($debian_chroot)──}${VIRTUAL_ENV:+(\[\033[0;1m\]$(basename $VIRTUAL_ENV)'$prompt_color')}('$info_color'\u${prompt_symbol}\h'$prompt_color')-[\[\033[0;1m\]\w'$prompt_color']\n'$prompt_color'└─'$info_color'\$\[\033[0m\]\[\033[?112;0;64c\] '
		;;
	oneline)
...
# alias appimages, if there are any
if [ -f "$HOME/appimages/nvim.appimage" ]; then
...
export PATH="$HOME/bin:$HOME/.local/bin:/opt/gcc-eabi/bin:/usr/games:$HOME/perl5/bin${PATH:+:${PATH}}"
export GOPATH=$HOME/source/go
export PATH=$GOPATH/bin:$GOROOT/bin:$PATH:~/zig:~/appimages

# show local machine info if exists
exec_if_exists ~/.machineinfo

source /home/elsni/.bash_completions/nala.sh
